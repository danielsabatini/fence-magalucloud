...
all_opt = {
    ...
    'agent': {'getopt': '', 'help': '', 'order': 1},
    'web': {'getopt': '', 'help': '', 'order': 1},
    'force_on': {'getopt': '', 'help': '', 'order': 1},
    'action': {
        'getopt': 'o:',
        'longopt': 'action',
    ...
    'fabric_fencing': {'getopt': '', 'help': '', 'order': 1},
    'ipaddr': {
        'getopt': 'a:',
        'longopt': 'ip',
    ...
    'no_login': {'getopt': '', 'help': '', 'order': 1},
    'no_password': {'getopt': '', 'help': '', 'order': 1},
    'no_port': {'getopt': '', 'help': '', 'order': 1},
    'no_status': {'getopt': '', 'help': '', 'order': 1},
    'no_on': {'getopt': '', 'help': '', 'order': 1},
    'no_off': {'getopt': '', 'help': '', 'order': 1},
    'telnet': {'getopt': '', 'help': '', 'order': 1},
    'diag': {'getopt': '', 'help': '', 'order': 1},
    'passwd': {
        'getopt': 'p:',
        'longopt': 'password',
...
def _add_dependency_options(options):
    ...
    if (
        ...
        and 'nodename' not in (options + added_opt)
        and 'ipaddr' in (options + added_opt)
    ):
        added_opt.append('port_as_ip')
        all_opt['port']['help'] = (
            '-n, --plug=[ip]                IP address or hostname of fencing device '
...
def metadata(options, avail_opt, docs, agent_name=os.path.basename(sys.argv[0])):
    ...
    print('<resource-agent name="' + agent_name + '" shortdesc="' + docs['shortdesc'] + '" >')
...
def check_input(device_opt, opt, other_conditions=False):
    ...
    if options['--action'] in ['metadata', 'manpage'] or any(
        k in options for k in ('--help', '--version')
    ):
        return options
    ...
    if options['--verbose-level'] < 0:
        logging.warning(
            "Parse error: Option 'verbose_level' must "
    ...
    if options['--verbose-level'] == 0 and '--verbose' in options:
        logging.warning(
            "Parse error: Ignoring option 'verbose' because it conflicts with verbose_level=0"
        )
        del options['--verbose']
...
def fence_action(
    ...
    try:
                ...
                for outlet_id in list(outlets.keys()):
                    ...
                    if options['--action'] == 'list':
                        try:
                            ...
                            print(
                                '{}{}{}'.format(outlet_id, options['--separator'], alias).encode(
                                    'utf-8'
                                )
                            )
        ...
        if (
            options['--action'] == 'monitor'
            and 'port' not in options['device_opt']
            and 'no_status' in options['device_opt']
        ):
        ...
        if options['--action'] == 'on':
            ...
            if set_multi_power_fn(
                connection, options, set_power_fn, get_power_fn, sync_set_power_fn
            ):
            ...
            if options.get('--method', '').lower() == 'cycle' and reboot_cycle_fn is not None:
                try:
                    power_on = multi_reboot_cycle_fn(
                        connection, options, reboot_cycle_fn, 1 + int(options['--retry-on'])
                    )
...
def _open_ssl_connection(options):
    ...
    if '--notls' in options:
        ...
        gnutls_opts = (
            '--priority "NORMAL:-VERS-TLS1.2:-VERS-TLS1.1:+VERS-TLS1.0:%LATEST_RECORD_VERSION"'
        )
...
def _update_metadata(options):
    ...
    if device_opt.count('ipport'):
        ...
        if default_string is None:
            all_opt['ipport']['help'] = (
                '-u, --ipport=[port]            TCP/UDP port to use (default %s)' % (default_value)
            )
        else:
            all_opt['ipport']['help'] = (
...
def _validate_input(options, stop=True):
    ...
    for failed_opt in _get_opts_with_invalid_choices(options):
        valid_input = False
        fail_usage(
            'Failed: You have to enter a valid choice for %s from the valid values: %s'
    ...
    for failed_opt in _get_opts_with_invalid_types(options):
        ...
        if all_opt[failed_opt]['type'] == 'second':
            fail_usage(
                'Failed: The value you have entered for %s is not a valid time in seconds'
                % ('--' + all_opt[failed_opt]['longopt']),
                stop,
            )
        else:
            fail_usage(
                'Failed: The value you have entered for %s is not a valid %s'
...
def _parse_input_stdin(avail_opt):
    ...
    for line in sys.stdin.readlines():
        ...
        if all_opt[name]['getopt'].endswith(':'):
            ...
            logging.warning(
                "Parse error: Ignoring option '%s' because it does not have value\n", name
            )
...
def _parse_input_cmdline(avail_opt):
    ...
    if 'port_as_ip' in avail_opt and '--port-as-ip' not in long_opts and '--plug' in long_opts:
    ...
    for opt in device_opt:
        if 'choices' in all_opt[opt]:
            ...
            if longopt in options:
                ...
                if options['--' + all_opt[opt]['longopt']] not in possible_values_upper:
